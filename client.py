...
class Client():
    ...
    def Signup(self, roots):  # This is the signup definition,
        ...
        pwordE = Entry(roots,show='*',width=30)# Same as above, yet 'show="*"' What this does is replace the text with *, like a password box :D
        ...
        submit_Button = Button(roots, text='submit',width=25,bg="DodgerBlue2",
                               command=lambda: self.chack_Signup(roots)) # This creates the button with the text 'signup', when you click it, the command 'fssignup' will run. which is the def
        print ("after")
    ...
    def chack_Signup(self, roots):
        ...
        print()
        if password == re_password and username != "" and email != "" and password != ""and re_password != "" and len(password)>5:
    ...
    def Login(self,roots):
        ...
        loginB = Button(roots, text='Login',
                        command=lambda: self.CheckLogin(roots))  # This makes the login button, which will go to the CheckLogin def.
        loginB.grid(columnspan=2, sticky=W)
        signupB = Button(roots, text="Signup",
                        command=lambda: self.Signup(roots))  # This makes the login button, which will go to the CheckLogin def.
